...
class TestPeerList(unittest.TestCase):
    def setUp(self):
        endpoints = [f'rest-api-{i}:8008' for i in range(len(EXPECTED))]
    ...
    def test_peer_list(self):
        ...
        subprocess.run(
            shlex.split(f'sawnet peers graph --force {http_addresses}'), check=True
        )
...
def _make_http_address(node_number):
    return f'http://rest-api-{node_number}:8008'


def _make_tcp_address(node_number):
    return f'tcp://validator-{node_number}:8800'
