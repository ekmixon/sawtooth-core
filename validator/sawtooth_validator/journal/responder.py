...
class Responder:
    ...
    def check_for_block(self, block_id):
        return (
            self.completer.get_chain_head()
            if block_id == "HEAD"
            else self.completer.get_block(block_id)
        )

    def check_for_batch(self, batch_id):
        return self.completer.get_batch(batch_id)

    def check_for_batch_by_transaction(self, transaction_id):
        return self.completer.get_batch_by_transaction(transaction_id)

    def already_requested(self, requested_id):
        with self._lock:
            return requested_id in self.pending_requests
...
class BatchByBatchIdResponderHandler(Handler):
    ...
    def handle(self, connection_id, message_content):
        ...
        if batch is None:
            ...
            # and add to pending requests
            if self._responder.already_requested(batch_request_message.id):
                ...
                self._responder.add_request(batch_request_message.id,
                                            connection_id)
            elif batch_request_message.time_to_live > 0:
...
class BatchByTransactionIdResponderHandler(Handler):
    ...
    def handle(self, connection_id, message_content):
        ...
        if not batches and len(not_requested) == len(batch_request_message.ids):
